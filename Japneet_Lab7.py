...
assert reverse_str("Rainbow") == "wobniaR"
assert reverse_str("Lab7") == "7baL"
assert reverse_str("project1!") == "!1tcejorp"
...
assert remove_duplicates("pet") == "pet"
assert remove_duplicates("bearr") == "bear"
assert remove_duplicates("oops!") == "ops!"
...
assert find_index("Banana", 'p') == -1
assert find_index("Comp115", 'm') == 2
...
def project_completion_day(day, days_to_completion):
    ...
    Write your docstring
    """
    index_begin = days_week.index(day)
    index_complete = (index_begin + days_to_completion) % 7

    return days_week[index_complete]

assert project_completion_day("Monday", 2) == 'Wednesday'
assert project_completion_day("Thursday", 3) == 'Sunday'
assert project_completion_day("Friday", 1) == 'Saturday'
...
assert parse_log_line(line) == ('2024-03-05 14:32:18', 'WARNING', 'api.py', 'Slow query detected (2.3s)')
